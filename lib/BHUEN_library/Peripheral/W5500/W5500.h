...
/***************----- W5500 GPIO定义 -----***************/
	#define W5500_SCS_PIN		A4	//定义W5500的CS引脚，片选低有效 
	//#define W5500_SCS_PORT	GPIOA
		
	#define W5500_RST_PIN		A5	//定义W5500的RST引脚，低500us有效
	//#define W5500_RST_PORT	GPIOC

	#define W5500_INT_PIN		A6	//中断输出，低有效
	//#define W5500_INT_PORT	GPIOC
	#define W5500_SCLK_PIN  A7  //spi时钟输入
	#define W5500_MOSI_PIN  A0  //主机输出从机输入
	#define W5500_MISO_PIN  A1  //主机输入从机输出
/***************----- 网络参数变量定义 -----***************/
...
extern uint8_t Detect_Gateway(void);//检查网关服务器
extern void Socket_Init(SOCKET s);//指定Socket(0~7)初始化
extern uint8_t Socket_Connect(SOCKET s);//设置指定Socket(0~7)为客户端与远程服务器连接
extern uint8_t Socket_Listen(SOCKET s);//设置指定Socket(0~7)作为服务器等待远程主机的连接
extern unsigned char Socket_UDP(SOCKET s);//设置指定Socket(0~7)为UDP模式
extern uint16_t Read_SOCK_Data_Buffer(SOCKET s, uint8_t *dat_ptr);//指定Socket(0~7)接收数据处理
...
extern void W5500_Initialization(void);//W5500初始化配置
extern void Load_Net_Parameters(void);//装载网络参数
extern void W5500_Socket_Set(void);//W5500端口初始化配置
extern void Process_Socket_Data(SOCKET s);//W5500接收并发送接收到的数据
extern void socket_close(SOCKET s);//关闭scoket
